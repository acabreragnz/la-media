...
const disclaimerDismissed = useLocalStorage('itaumedia_disclaimer_dismissed', false)
</script>

<template>
  <nav>
    <RouterLink to="/">ITAU</RouterLink>
    <RouterLink to="/itau">ITAU</RouterLink>
  </nav>
    ...
    <div class="w-full max-w-[440px]">
      ...
      <div v-if="loading && !rates" class="space-y-5">
        ...
        <div class="flex items-center justify-center gap-3 mb-7">
          <a href="https://www.itau.com.uy/inst/aci/cotiz.xml" target="_blank" rel="noopener noreferrer" class="hover:opacity-80 transition-opacity">
            <img src="/brou-logo.webp" alt="ITAU" class="h-10 w-auto" />
          </a>
        ...
        <div class="relative h-[2px] mb-6 overflow-hidden">
          <div class="absolute inset-0 bg-gradient-to-r from-transparent via-itau-yellow/60 to-transparent blur-[1px]"></div>
          <div class="absolute inset-0 bg-gradient-to-r from-transparent via-itau-yellow/40 to-transparent"></div>
        </div>
          ...
          <div class="bg-white/[0.04] rounded-xl px-5 py-3 border border-white/[0.05] animate-pulse relative">
            <div class="flex items-center justify-center gap-3">
              ...
              <div class="flex flex-col items-center gap-1">
                ...
                <div class="h-7 bg-itau-yellow/20 rounded w-16"></div>
              </div>
              <!-- Separador -->
              <div class="h-4 bg-white/[0.05] rounded w-1"></div>
              <!-- Venta skeleton -->
        ...
        <div class="flex justify-center my-4">
          <div class="w-12 h-12 bg-gradient-to-br from-itau-blue/50 to-itau-blue-light/50 rounded-full animate-pulse"></div>
        </div>

        <!-- Result Display skeleton -->
        <div class="bg-gradient-to-br from-itau-blue/10 to-itau-blue-light/8 border border-itau-blue-light/20 rounded-[20px] p-5 animate-pulse">
          <div class="flex items-center gap-4">
        ...
        <div class="bg-white/[0.03] backdrop-blur-lg rounded-2xl p-4 border border-white/[0.08] animate-pulse">
          <div class="space-y-3">
            ...
            <div class="h-10 bg-itau-blue/5 border border-itau-blue-light/20 rounded-xl"></div>
          </div>
        </div>
      </div>
      ...
      <div v-else class="space-y-5">
        ...
        <div v-if="error" class="bg-yellow-500/10 border border-yellow-500/30 rounded-xl p-3 flex items-start gap-3">
          ...
          <div class="flex-1">
            ...
            <p class="text-white/70 text-[0.7rem] leading-relaxed">
              {{ rates ? 'No pudimos obtener datos actualizados desde itaú  . Mostrando última cotización disponible.' : error }}
            </p>
          </div>
          <button
            @click="() => refetch()"
        ...
        <div v-if="DISCLAIMER_ENABLED && !disclaimerDismissed" class="bg-yellow-500/10 border-l-4 border-yellow-500 rounded-lg p-4 mb-5 relative">
          ...
          <div class="flex items-start gap-3 pr-8">
            ...
            <div class="flex-1">
              <h3 class="text-yellow-400 font-bold text-[0.9rem] mb-1">Sitio NO oficial de ITAU</h3>
              <p class="text-white/80 text-[0.75rem] leading-relaxed mb-2">
                Este conversor <strong>no está afiliado, asociado ni respaldado</strong> por el Banco de la República Oriental del Uruguay (ITAU).
              </p>
              <p class="text-white/80 text-[0.75rem] leading-relaxed">
                Los datos pueden contener errores o estar desactualizados. <strong>No nos hacemos responsables</strong> por pérdidas derivadas del uso de esta información. Verifica siempre en <a href="https://www.itau.com.uy/inst/aci/cotiz.xml" target="_blank" rel="noopener" class="text-yellow-400 underline font-semibold">itau.com.uy</a>
              </p>
            </div>
          </div>
        </div>
          ...
          <div class="bg-white/[0.04] rounded-xl px-5 py-3 text-sm border border-white/[0.05] relative">
            ...
            <div class="flex items-center justify-center gap-3">
              ...
              <div class="flex flex-col items-center">
                <span class="text-itau-yellow/80 text-xs uppercase tracking-wider font-medium">Media</span>
                <span v-if="rates" class="text-itau-yellow font-bold text-2xl">
            ...
            <!-- Enlace ITAU -->
            <a
              href="https://www.itau.com.uy/inst/aci/cotiz.xml"
              target="_blank"
        ...
        <label
          ...
          class="bg-white/[0.05] border border-white/[0.08] rounded-[20px] p-5 flex items-center gap-4 transition-all focus-within:border-itau-blue-light/60 focus-within:bg-itau-blue/[0.06] focus-within:shadow-[0_0_30px_rgba(8,82,141,0.25)] cursor-text"
        >
        ...
        <div class="flex justify-center my-4">
          <button
            @click="swapDirection"
            class="flex items-center justify-center w-12 h-12 bg-gradient-to-br from-itau-blue to-itau-blue-light rounded-full border-none cursor-pointer transition-all hover:scale-110 active:scale-95 shadow-[0_8px_20px_rgba(8,82,141,0.5),0_0_40px_rgba(13,94,161,0.3)] hover:shadow-[0_12px_30px_rgba(8,82,141,0.7),0_0_60px_rgba(13,94,161,0.5)]"
        ...
        <!-- Result Display -->
        <div class="bg-gradient-to-br from-itau-blue/10 to-itau-blue-light/8 border border-itau-blue-light/20 rounded-[20px] p-5 shadow-[inset_0_1px_0_rgba(255,255,255,0.05),0_0_40px_rgba(8,82,141,0.15)] flex items-center gap-4">
          <span class="text-[2.25rem] leading-none">{{ direction === 'usdToUyu' ? '🇺🇾' : '🇺🇸' }}</span>
          ...
          <div class="text-center space-y-2.5">
            ...
            <div v-if="DISCLAIMER_ENABLED" class="text-[0.55rem] sm:text-[0.6rem] text-white/60 flex items-start justify-center gap-1 leading-tight">
              ...
              <span class="text-left sm:text-center">
                Sitio no oficial. <a href="https://www.itau.com.uy/inst/aci/cotiz.xml" target="_blank" rel="noopener" class="text-itau-yellow/80 hover:underline font-medium">Verificar en ITAU</a>
              </span>
            </div>
            ...
            <button
              ...
              class="flex items-center justify-center gap-2 w-full py-2.5 px-4 bg-itau-blue/5 border border-itau-blue-light/40 rounded-xl text-white text-[0.8rem] font-semibold cursor-pointer transition-all hover:bg-itau-blue/10 hover:border-itau-blue-light/60"
      ...
      <div class="text-center mt-6 text-white/60 text-[0.8rem]">
        ...
        <a href="https://github.com/acabreragnz"
           target="_blank"
           rel="noopener"
           class="text-itau-blue-light/90 no-underline font-medium transition-colors hover:text-itau-yellow hover:underline decoration-itau-yellow decoration-2">
          @acabreragnz
        </a>
      </div>
    </div>
...
<style scoped>
/* Gradiente mobile: azul ITAU en diagonal superior derecha */
.app-gradient {
